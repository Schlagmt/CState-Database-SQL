...
FROM
	TFlights AS TF
	,TFlightPilots AS TFP
	,TPilots AS TP	
	,TAirplanes AS TA
WHERE
	TF.intFlightID = TFP.intFlightID
AND	TFP.strPilotRole = 'Pilot'
AND	TFP.intPilotID = TP.intPilotID
AND TF.intAirplaneID = TA.intAirplaneID
ORDER BY
	TF.strFlight
-- --------------------------------------------------------------------------------
...
FROM
	TFlights AS TF
	,TFlightPassengers AS TFP
	,TPassengers AS TP
WHERE
	TF.intFlightID = TFP.intFlightID
AND TFP.intPassengerID = TP.intPassengerID
ORDER BY
	TF.strFlight